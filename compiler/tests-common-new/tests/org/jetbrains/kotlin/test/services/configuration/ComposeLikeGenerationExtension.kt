    ...
    override fun visitCall(expression: IrCall): IrExpression {
        ...
        return if (rewrittenFunctions.contains(function)) {
            IrCallImpl(
                ...
                var bitmap = 0
                function.parameters.dropLast(1).zip(expression.arguments).forEachIndexed { i, (parameter, argument) ->
                    if (argument != null) {
                        it.arguments[i] = argument.applyIf(
                            parameter.kind==IrParameterKind.DispatchReceiver || parameter.kind==IrParameterKind.ExtensionReceiver
                        ) {
                            transform(this@ComposeLikeDefaultMethodCallRewriter, null)
                        }
                    } else {
                ...
                it.arguments[function.parameters.size - 1] =
