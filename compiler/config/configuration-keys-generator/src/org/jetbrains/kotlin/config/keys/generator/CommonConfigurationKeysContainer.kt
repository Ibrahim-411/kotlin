...
object CommonConfigurationKeysContainer : KeysContainer("org.jetbrains.kotlin.config", "CommonConfigurationKeys") {
    ...
    val LOOKUP_TRACKER by key<LookupTracker>("lookup tracker", alwaysNullable = true)
    val EXPECT_ACTUAL_TRACKER by key<ExpectActualTracker>("expect actual tracker", alwaysNullable = true)
    val INLINE_CONST_TRACKER by key<InlineConstTracker>("inline constant tracker", alwaysNullable = true)
    val ENUM_WHEN_TRACKER by key<EnumWhenTracker>("enum when tracker", alwaysNullable = true)
    val IMPORT_TRACKER by key<ImportTracker>("import tracker", alwaysNullable = true)
