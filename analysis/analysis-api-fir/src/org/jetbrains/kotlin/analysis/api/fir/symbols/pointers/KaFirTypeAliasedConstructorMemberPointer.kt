...
internal class KaFirTypeAliasedConstructorMemberPointer(
    ...
    private fun getSearchScope(analysisSession: KaFirSession, typeAlias: FirTypeAliasSymbol): FirScope? {
        // TODO Consider extracting code above to a function and reusing it in other places
        val firSession = analysisSession.firSession
        val scopeSession = analysisSession.getScopeSessionFor(firSession)

        val expandedType = typeAlias.resolvedExpandedTypeRef.coneTypeSafe<ConeClassLikeType>()?.fullyExpandedType(firSession) ?: return null
        val outerType = outerType(expandedType, firSession) { it.getContainingDeclaration(firSession) }

        val basicScope = expandedType.scope(
            firSession,
            scopeSession,
        ...
        return TypeAliasConstructorsSubstitutingScope(typeAlias, basicScope, outerType)
    }
    ...
    private fun hasTheSameOwner(other: KaFirTypeAliasedConstructorMemberPointer): Boolean =
